...
//These are the cases in the pattern matching
abstract class Rexp
case object ZERO extends Rexp                 // matches nothing
case object ONE extends Rexp                  // matches the empty string
case class CHAR(c: Char) extends Rexp         // matches a character c
case class ALTs(rs: List[Rexp]) extends Rexp  // alternatives | basically means OR
case class SEQs(rs: List[Rexp]) extends Rexp  // sequences ~ two next to each other
case class STAR(r: Rexp) extends Rexp         // star % repeated as many times
...
def der (c: Char, r: Rexp) : Rexp = r match {
  ...
  case SEQs(rs) if rs.isEmpty => ZERO
  case SEQs(r::rs) if nullable(r) => ALT(SEQs(der(c, r) :: rs), der(c, SEQs(rs)))
  case SEQs(r::rs) => SEQs(der(c, r) :: rs)
  case STAR(r) => SEQ(der(c, r) , STAR(r))
}

// (3) Denest is for ALTs
...
def flts(rs: List[Rexp], acc: List[Rexp] = Nil) : List[Rexp] = rs match {
  case Nil => acc
  case ZERO::rest => List[Rexp](ZERO)
  case ONE::rest => flts(rest, acc)
  case SEQs(rs)::rest => flts(rest, acc:::rs)
  case r::rest => flts(rest, acc:::List[Rexp](r))
}
...
def simp(r: Rexp) : Rexp = r match {
  case ALTs(rs) => ALTs_smart(denest(rs.map(r => simp(r))).distinct)
  case SEQs(rs) => SEQs_smart(flts(rs.map(r => simp(r))))
  case r => r
}
...
def size(r: Rexp): Int = r match {
  ...
  case ONE => 1
  case CHAR(c) => 1
  case ALTs(rs) => 1 + rs.map(a => size(a)).sum
  case SEQs(rs) => 1 + rs.map(a => size(a)).sum
